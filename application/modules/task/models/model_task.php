...
class Model_task extends CI_Model
{
	...
	function get_main_list($offset = 0, $limit = 10, $order = 'updated_at desc', $search = '', $category_id_list = array(), $with_logical_deleted = false, $columns = 'A.*, B.name, B.sub_id')
	{
	...
	private static function get_main_query($search = '', $category_id_list = array(), $is_count = false, $with_logical_deleted = false, $columns = 'A.*, B.name, B.sub_id')
	{
		...
		$select = sprintf("SELECT %s FROM task A", $columns);
		if ($is_count) $select = "SELECT COUNT(A.id) as count FROM task A";

		$sql = $select." LEFT JOIN task_category B ON A.task_category_id = B.id";
...
*/
	...
	{
		$CI =& get_instance();
		$row = $CI->db_util->get_row4id('task', $id, array('del_flg'), 'task');
	...
	public function update($values, $wheres, $update_datetime = true)
	{
		...
		return $this->db->update('task', $values, $wheres);
	}
