...
{form_open action="`$current_module`/execute_insert" id=main_form_activity}
...
{if $items.type == 'text'}
	<input type="text" name="{$key}"{if $items.value} value="{$items.value}"{/if} id="{if $items.id_name}{$items.id_name}{else}{$key}{if}"{$smarty.capture.item_class|smarty:nodefaults}{if $smarty.capture.form_help} placeholder="{$smarty.capture.form_help}"{/if}>{if $items.after_label}<span class="after_label">{$items.after_label}</span>{/if}
...
{/foreach}
<button id="main_form_submit_activity" class="iPhoneButton">送信</button>
{form_close}
